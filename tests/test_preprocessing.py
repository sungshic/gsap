...
def test_alignPairedEndReads(gsap_test_common):
    ...
    shutil.copyfile(
        test_ref_data_dir + "addrg_reads.bam", tmp_test_data_dir + "addrg_reads.bam"
    )
...
def test_sortSAMIntoBAM(gsap_test_common):
    pre_processor, test_ref_data_dir, tmp_test_data_dir, teardown = gsap_test_common
    shutil.copyfile(
        test_ref_data_dir + "aligned_reads.sam", tmp_test_data_dir + "aligned_reads.sam"
    )
    tmp_output_file_path = tmp_test_data_dir + "sorted_reads.bam"
    pre_processor.sortSAMIntoBAM(
        tmp_test_data_dir + "aligned_reads.sam", "sorted_reads.bam"
    )
    assert os.path.exists(tmp_output_file_path) == True
...
def test_removeUnmappedReads(gsap_test_common):
    pre_processor, test_ref_data_dir, tmp_test_data_dir, teardown = gsap_test_common
    shutil.copyfile(
        test_ref_data_dir + "sorted_reads.bam", tmp_test_data_dir + "sorted_reads.bam"
    )
    pre_processor.removeUnmappedReads(
        tmp_test_data_dir + "sorted_reads.bam",
        tmp_test_data_dir + "filtered_sample_aligned.bam",
        tmp_test_data_dir + "unmapped_reads.bam",
    )
...
def test_buildBamIndex(gsap_test_common):
    ...
    pre_processor.buildBamIndex(
        tmp_test_data_dir + "sorted_reads.bam", tmp_test_data_dir + "sorted_reads.bai"
    )
...
def test_buildRefFastaIndex(gsap_test_common):
    ...
    pre_processor.buildRefFastaIndex(
        ...
        tmp_test_data_dir + "AL009126.dict",
    )
