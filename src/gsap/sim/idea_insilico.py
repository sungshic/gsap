...
class PopulationManager:
    ...
    def generateIndividual(self, snp_idx_list : list[int]) -> list[list[int]]:  # , generation=None):
        ...
        ind_pattern = self._p_gen.generatePermutation(
            perm_len
    ...
    def mutateIndividual(self, ind, mut_prob):
        ...
        for idx in range(self._snp_m._max_snp_idx + 1):
            ...
            if idx in c_state_idx_list:
                ...
                ind[c_idx_idx] = (
                    ind[c_idx_idx][0],
                    self._p_gen.generatePermutation(1)[0],
                )
            else:
    ...
    def determineCrossoverRange(self, uc_state_idx_list, idx_tuple):
        ...
        range_minmax_diff = (
            uc_state_idx_list[idx_tuple[1]] - uc_state_idx_list[idx_tuple[0]]
        )
    ...
    def crossoverSNPAlleles(self, ind1, ind2, cx_prob):
        ...
        ind1_unconstrained_idx_set = set(
            range(min(ind1_state_idx_set) - 1, max(ind1_state_idx_set) + 2)
        ).difference(ind1_state_idx_set)
        ind2_unconstrained_idx_set = set(
            range(min(ind2_state_idx_set) - 1, max(ind2_state_idx_set) + 2)
        ).difference(ind2_state_idx_set)
        uc_state_idx_union = ind1_unconstrained_idx_set.union(
            ind2_unconstrained_idx_set
        )
        ...
        for idx_tuple in idx_tuple_list:
            ...
            if cx_range:
                ...
                ind1_c_state_idxs = [
                    idx
                    for idx in range(cx_idx_min, cx_idx_max + 1)
                    if idx in ind1_state_idx_set
                ]
                ind2_c_state_idxs = [
                    idx
                    for idx in range(cx_idx_min, cx_idx_max + 1)
                    if idx in ind2_state_idx_set
                ]
                ...
                if ind1_c_state_idxs:
                    ...
                    ind1_cx_idxs = [
                        idx_idx
                        for idx_idx, entry in enumerate(ind1)
                        if entry[0] in ind1_c_state_idxs
                    ]
                if ind2_c_state_idxs:
                    #                     crossover_range2 = ind2[min(ind2_c_state_idxs):max(ind2_c_state_idxs)+1]
                    ind2_cx_idxs = [
                        idx_idx
                        for idx_idx, entry in enumerate(ind2)
                        if entry[0] in ind2_c_state_idxs
                    ]
...
class PermPatternGenerator:
    ...
    def __init__(self, state_list, unit_len):
        ...
        self._lut[unit_len] = np.array(
            list(itertools.product(self._state_list, repeat=unit_len))
        )
        for short_split in range(1, unit_len):
            self._lut[short_split] = np.array(
                list(itertools.product(self._state_list, repeat=short_split))
            )
    ...
    def generatePermutation(self, perm_len):
        ...
        split_idx_list = range(perm_len)[
            :: self._unit_len
        ]  # splitting a perm_len long array into _unit_len long chunks
